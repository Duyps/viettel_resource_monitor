...
class TestSuiteHome extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Viettel Monitor - Test Suite', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Colors.white)),
        backgroundColor: const Color(0xFFEE0000),
      ),
      body: ListView(
        padding: const EdgeInsets.all(16),
        children: [
          const Text('Chọn một kịch bản để kiểm thử các cảnh báo tài nguyên của hệ thống:', style: TextStyle(fontSize: 16)),
  ...
  Widget _buildTestCard(BuildContext context, {required String title, required IconData icon, required Color color, required String route, required WidgetBuilder builder}) {
    return Card(
      elevation: 2,
    ...
    return Scaffold(
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) {
          // Simulate very heavy synchronous rendering task
          if (index % 5 == 0) {
            for (var i = 0; i < 800000; i++) {} // Heavy loop blocks UI thread
          }
          return ListTile(
  ...
  void _leakMemory() {
    setState(() {
      for (int i = 0; i < 500000; i++) {
        _leakedMemory.add('This is a very long string used to consume memory quickly to test the Viettel Resource Monitor Memory tracking feature... $i');
      }
    });
    debugPrint('ViettelResourceMonitor: Appended 500,000 strings to memory list.');
  }
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          ...
          children: [
            Text('Objects in Memory: ${_leakedMemory.length}', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
...
class _CpuTestScreenState extends State<CpuTestScreen> {
  ...
  void _startHeavyComputation() async {
    ...
    // Giả lập tính toán phức tạp trên luồng chính để đẩy CPU lên cao
    await Future.delayed(const Duration(milliseconds: 100)); // Nhường frame cho UI vẽ nút loading
...
class _NetworkTestScreenState extends State<NetworkTestScreen> {
  ...
  Future<void> _fetchFastApi() async {
    ...
    try {
      ...
      setState(() => _status = 'API Nhanh (200 OK): ${res.body.length} bytes');
    } catch (e) {
  ...
  Future<void> _fetchSlowApi() async {
    setState(() => _status = 'Đang gọi API chậm (delay 3s)...');
    try {
      // Dùng httpstat.us để giả lập mạng chậm 3 giây
      final res = await http.get(Uri.parse('https://httpstat.us/200?sleep=3000'));
      setState(() => _status = 'API Chậm (200 OK) sau 3s: ${res.body.length} bytes');
    } catch (e) {
      setState(() => _status = 'Lỗi: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Network Latency Test')),
      body: Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
