...
class ViettelDataAnalyzer {
  ...
  ViettelDataAnalyzer(this._config, this._onAlertCallback, this._alertStreamController);
  ...
  void analyzeImmediateResource(ResourceMetric metric, String screenName) {
    if (metric.memoryUsageMB > _config.maxMemoryMB) {
      _emitAlert(ResourceAlert(
        screenName: screenName,
        alertType: AlertType.memoryLeak,
        message: 'Cảnh báo: RAM vọt lên mức nguy hiểm (${metric.memoryUsageMB.toStringAsFixed(1)}MB > ${_config.maxMemoryMB}MB)',
    ...
    if (metric.cpuUsagePercentage > _config.maxCpuPercentage) {
      _emitAlert(ResourceAlert(
        ...
        message: 'Cảnh báo: CPU quá tải (${metric.cpuUsagePercentage.toStringAsFixed(1)}% > ${_config.maxCpuPercentage}%)',
        timestamp: metric.timestamp,
      ));
    }
  }
  ...
  void analyzeImmediateNetwork(NetworkMetric metric, String screenName) {
    if (metric.durationMilliseconds > _config.maxNetworkDurationMs) {
      _emitAlert(ResourceAlert(
        ...
        message: 'API [${metric.method}] phản hồi quá chậm: ${metric.durationMilliseconds}ms (Ngưỡng: ${_config.maxNetworkDurationMs}ms)\nURL: ${metric.url}',
        timestamp: metric.timestamp,
      ));
    }
  }
  ...
  void analyzeSession(ScreenSession session) {
    ...
    for (var metric in session.resourceMetrics) {
      ...
      if (metric.cpuUsagePercentage > maxCpu) maxCpu = metric.cpuUsagePercentage;
    }
    ...
    if (avgFps < _config.minFps) {
      _emitAlert(ResourceAlert(
        ...
        message: 'Cảnh báo Jank! Màn hình giật lag với FPS trung bình chỉ: ${avgFps.toStringAsFixed(1)} (Dưới ${_config.minFps})',
    ...
    if (kDebugMode) {
      debugPrint('ViettelResourceMonitor: [Summary ${session.screenName}] AvgFPS: ${avgFps.toStringAsFixed(1)}, PeakRAM: ${maxRam.toStringAsFixed(1)}MB, PeakCPU: ${maxCpu.toStringAsFixed(1)}%');
    }
  }
}
